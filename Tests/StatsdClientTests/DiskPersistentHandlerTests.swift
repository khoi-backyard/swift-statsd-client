...
class DiskPersistentHandlerTests: XCTestCase {
    ...
    func testInitialization() {
        XCTAssertNoThrow(try DiskPersistentHandler(config: DiskConfiguration()), "Able to initialize DiskPersistentHandler with default configuration")
    }
    ...
    func testTotalCountAfterInitializationBefore() {
        ...
        XCTAssertNoThrow(try handler.write(metric, key: metric.name, attribute: nil), "Write fill successfully without any execptions")

        let anotherHandler = try! DiskPersistentHandler(config: config, hanlder: fileManager)
        XCTAssertEqual(anotherHandler.getTotal(), 1, "Another Hanlder should fetch same data if same configuration")
    }

    func testCreateCacheFolderAfterInitalization() {
        let isExisted = fileManager.fileExists(atPath: config.pathFolder)
        XCTAssertTrue(isExisted, "After initialized successfully, it should create cache folder which cooresponse with config's pathFolder")
    }
    ...
    func testMakePathFile() {
        ...
        let expected = "\(config.pathFolder)/\(fileName.toBase64())"
    ...
    func testWriteIndividualFile() {
        ...
        XCTAssertNoThrow(try handler.write(metric_1, key: metric_1.name, attribute: nil), "Write fill successfully without any execptions")
        XCTAssertEqual(handler.getTotal(), 1, "File count should be 1")
        XCTAssertNoThrow(try handler.write(metric_2, key: metric_2.name, attribute: nil), "Write fill successfully without any execptions")
    ...
    func testReceivedFile() {
        ...
        XCTAssertThrowsError(try self.handler.get(key: "Wrong key", type: StubMetric.self), "Should throw exception because fetching no exist file")
    }
